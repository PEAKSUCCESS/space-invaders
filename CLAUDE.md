...
- **Decorative traffic** (pure scenery, behind the rocks, no pointer events): the **PeakESL cruiser** — a hull emblazoned with `public/peak_logo.png` (counter-mirrored on right-to-left passes so the wordmark stays readable) — glides across every ~12–30s, and small **rockets** zip through in random directions every ~5–14s. Timer-driven `Flyby` state in `Asteroids.tsx`; each removes itself after its crossing.
