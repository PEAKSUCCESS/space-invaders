...
- **Weighted `/next`** — lessons are built from `/bin` (one fetch; matching needs 5 words at once). Switching per-step picks to the weighted `/next` is a later refinement.
