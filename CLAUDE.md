...
- **Decorative traffic** (clickable scenery, behind the rocks): the **PeakESL cruiser** — a hull emblazoned with `public/peak_logo.png` (counter-mirrored on right-to-left passes so the wordmark stays readable) — glides across every ~12–30s in a lane **above or below the centre band, so it never crosses the player's ship**; clicking it pops a **2-second card in the cruiser's own shape, larger, reading "PEAK ESL — Speech is Power"**. Small **rockets** zip through in random directions every ~5–14s; **clicking a rocket explodes it** into shards (no gameplay effect). Timer-driven `Flyby` state in `Asteroids.tsx`; each removes itself after its crossing.
