...
- **Ship damage is visible**: battle scars accumulate on the ship SVG as the hull drops (scuffs < 75, crack + embers < 50, glowing breach + dulled outline < 25), and the hull number rides **directly under the ship** (colour-matched, non-rotating) in addition to the top-left HULL bar.
